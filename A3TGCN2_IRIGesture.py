...
loader = IRIGestureTemporal(os.path.join(Path().absolute(),'dataset'), 'Dynamic')
dataset = loader.getDynamicTemporalDataset()
# n frames
# 33 nodes
# 4 features
# 8 outputs
...
train_input = np.array(train_dataset.features) # (11562, 3, 21, 16) (192, 4, 33, 18)
train_input = np.transpose(train_input, (0,2,1,3)) # (11562, 21, 3, 16) (192, 33, 4, 18)
train_target = np.array(train_dataset.targets) # (11562, 16, 6) (192, 18, 7)
train_target = np.transpose(train_target, (0,2,1)) # (11562, 6, 16) (192, 7, 18)
...
class AttentionGAT(torch.nn.Module):
    def __init__(self, node_features, periods, batch_size):
        ...
        self.linear2 = torch.nn.Linear(33, 7)
...
model = AttentionGAT(node_features=4, periods=18, batch_size=batch_size).to(DEVICE)
...
for epoch in range(300):#(30, 60):
    ...
    torch.save(model.state_dict(), "C:\_Projects\IRIGesture\A3TGCN2_Checkpoints\EpochTemporal_"+str(epoch)+".pth")
